  ...
  <div class="ratingselect">
    <div class="rating-type border-1px">
      ...
      <span class="block" :class="{active: selectType===0}" @click="setSelectType(0)">
        推荐<span class="count">{{positiveContent}}</span>
      </span>
      <span class="block" :class="{active: selectType===1}" @click="setSelectType(1)">
        吐槽<span class="count">{{ratings.length - positiveContent}}</span>
      </span>
    </div>
    <div class="switch" :class="{on: onlyContent}" @click="toggleRatingContent">
...
<script>
  ...
  export default {
    ...
    methods: {
      setSelectType(type){  //点击切换满意，不满意，全部
